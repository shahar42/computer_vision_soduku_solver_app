...
def load_dataset(data_dir: str, prefix: str, as_numpy: bool = False) -> Tuple[List[Any], List[Any]]:
    """
    ...
    Args:
        ...
        prefix: Prefix for saved files
        as_numpy: Whether to return NumPy arrays (True) or lists (False)
    ...
    # Return as NumPy arrays or convert to lists
    if as_numpy:
        return data, labels
    else:
...
def extract_cells_from_images(
    ...
    for idx, (image, grid) in enumerate(zip(images, grids)):
        try:
            # Use the full pipeline processing method
            results = pipeline.process_image_data(image)
            
            # Get extracted cells from results - FIXED: removed 'steps' parameter
            cell_images = results.get("cell_images")  # Removed pipeline._extract_cells() call
