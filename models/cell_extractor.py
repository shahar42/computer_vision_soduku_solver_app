...
class PerspectiveCellExtractor(CellExtractorBase):
    ...
    def __init__(self):
        ...
        self.border_padding = self.settings.get("border_padding", 0.15)
        
        # Image enhancement options
        self.perspective_correction = self.settings.get("perspective_correction", True)
        self.contrast_enhancement = self.settings.get("contrast_enhancement", True)
        ...
        self.histogram_equalization = self.settings.get("histogram_equalization", True)
    ...
    def _enhance_cell(self, cell: ImageType) -> ImageType:
        ...
        if self.adaptive_thresholding:
            # Check if cell has sufficient contrast
            cell_min, cell_max = np.min(cell), np.max(cell)
            if cell_max - cell_min > 30:  # Only apply if there's reasonable contrast
                ...
                # If low contrast, try a different approach
        ...
        if self.contrast_enhancement:
            ...
            for i in range(256):
                lut[i] = np.clip(int(i * 1.2 - 30), 0, 255)
                
            # Apply lookup table
            cell = cv2.LUT(cell, lut)
